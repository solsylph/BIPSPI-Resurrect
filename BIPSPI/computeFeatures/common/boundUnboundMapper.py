...
class BoundUnboundMapper(object):
  ...
  '''
  scoreMat= substitution_matrices.load("BLOSUM62")
...
#    print(boundToUnboundResDict)
  ...
  def _alig_seq(self, seq1, seq2):
    ...
    aligner.mode = "local"
    aligner.substitution_matrix = BoundUnboundMapper.scoreMat
    aligner.open_gap_score = -11
    aligner.extend_gap_score = -0.5
    alignments = aligner.align(seq1, seq2)
    ali = alignments[0]
    s1_padded, s2_padded = _padded_local_alignment(seq1, seq2, ali)
    return s1_padded, s2_padded, ali.score
