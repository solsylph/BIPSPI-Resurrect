...
SCORE_MAT= substitution_matrices.load("BLOSUM62")
IDENTITY_THR= 0.95
...
class HomoOligomerFinderBase(object):
  ...
  def _alignSeqs(self, seq0, seq1):
    ...
    aligner.mode = "local"
    aligner.substitution_matrix = SCORE_MAT
    aligner.open_gap_score = -11
    aligner.extend_gap_score = -0.5
    alignments = aligner.align(seq0, seq1)
    out = []
    for ali in alignments:
      s0_padded, s1_padded = _padded_local_alignment(seq0, seq1, ali)
      out.append((s0_padded, s1_padded, ali.score))
    return out
...
class HomoOligomerFinderLR(HomoOligomerFinderBase):
  ...
  def update_interactions(self):
    ...
    equivResiduesRL_dict= {v: k for k, v in equivResiduesLR_dict.items()}
